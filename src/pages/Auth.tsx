...
export default function Auth() {
  useSEO(seoConfigs.authPL);
  
  const [searchParams] = useSearchParams();
  const navigate = useNavigate();
  const { user, signIn, signUp, signInWithGoogle, resetPassword, loading } = useAuthContext();
  
  const [activeTab, setActiveTab] = useState<'login' | 'register' | 'reset'>(
    searchParams.get('tab') === 'register' ? 'register' : 
    searchParams.get('tab') === 'reset' ? 'reset' : 'login'
  );
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      if (error.message.includes('Invalid login credentials')) {
        ...
        toast.error('Email nie został potwierdzony. Sprawdź swoją skrzynkę pocztową.');
      } else {
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (result.error) {
      toast.error(result.error.message || 'Błąd logowania przez Google');
    }
  };
        ...
        <Card className="glass glass-border">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl font-bold">
              {activeTab === 'login' && 'Zaloguj się'}
              {activeTab === 'register' && 'Utwórz konto'}
              {activeTab === 'reset' && 'Resetuj hasło'}
            </CardTitle>
            <CardDescription>
              {activeTab === 'login' && 'Wprowadź dane, aby kontynuować'}
              {activeTab === 'register' && 'Wypełnij formularz, aby utworzyć konto'}
              {activeTab === 'reset' && 'Podaj email powiązany z kontem'}
            </CardDescription>
          </CardHeader>
